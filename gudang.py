...
class Pergudangan:
    ...
    def keluarkan_barang(self, nama_barang, jumlah):
        for barang in self.list_barang:
            if barang.nama == nama_barang and barang.jumlah >= jumlah:
                ...
                self.transaksi.append(f"Keluarkan {jumlah} {nama_barang}.")
    ...
    def tampilkan_rincian_penjualan(self):
        if not self.penjualan:
            ...
            table = PrettyTable()
            table.field_names = ["Nama Barang", "Jumlah", "Total", "Uang Dibayar", "Kembalian", "Sisa Pembayaran"]
            table.align["Nama Barang"] = "l"
            table.align["Jumlah"] = "r"
            table.align["Total"] = "r"
            table.align["Uang Dibayar"] = "r"
            table.align["Kembalian"] = "r"
            table.align["Sisa Pembayaran"] = "r"
            table.horizontal_char = '-'
            table.vertical_char = '|'
            table.junction_char = '+'

            for item in self.penjualan:
                table.add_row([item.nama_barang, item.jumlah, item.total, item.uang_dibayar, item.kembalian, item.sisa_pembayaran])
...
def main():
    ...
    while True:
        ...
        print("3. Tampilkan Tabel Barang")
        print("4. Tampilkan Rincian Transaksi")
        print("5. Tampilkan Rincian Penjualan")
        print("6. Keluar")
